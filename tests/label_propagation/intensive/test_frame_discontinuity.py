...
def image_dataset(request):
    ...
    if "labels_test" in dataset._dataset.get_field_schema():
        dataset._dataset.delete_sample_field("labels_test", error_level=2)
...
def test_frame_discontinuity(image_dataset):
    ...
    for sequence in sequences:
        sequence_view = image_dataset.match_tags([sequence]).sort_by(
            "frame_number"
        )
        sequence_view.set_values(
            "labels_test",
            [
        ...
        first_image = cv2.cvtColor(
            cv2.imread(sequence_view[first_sample_id].filepath),
            cv2.COLOR_BGR2RGB,
        )
        ...
        for sample_id in sequence_ids[1:]:
            ...
            sample_image = cv2.cvtColor(
                cv2.imread(sequence_view[sample_id].filepath),
                cv2.COLOR_BGR2RGB,
            )
            (
                gray_correlation,
                hsv_correlation,
                gray_diff,
            ) = frame_discontinuity_test(first_image, sample_image)
            discontinuity_scores.append(
                (gray_correlation, hsv_correlation, gray_diff)
            )
        ...
        logger.info(
            f"gray_correlation: min = {np.min(GREY_CORRELATION_THRESHOLDS)}, max = {np.max(GREY_CORRELATION_THRESHOLDS)}, median = {np.median(GREY_CORRELATION_THRESHOLDS)}"
        )
        logger.info(
            f"hsv_correlation: min = {np.min(HSV_CORRELATION_THRESHOLDS)}, max = {np.max(HSV_CORRELATION_THRESHOLDS)}, median = {np.median(HSV_CORRELATION_THRESHOLDS)}"
        )
        logger.info(
            f"gray_diff: min = {np.min(GRAY_DIFF_THRESHOLDS)}, max = {np.max(GRAY_DIFF_THRESHOLDS)}, median = {np.median(GRAY_DIFF_THRESHOLDS)}"
        )
